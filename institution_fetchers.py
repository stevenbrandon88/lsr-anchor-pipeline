...
def fetch_wb_ieg(max_rows=None, force_local=False) -> pd.DataFrame:
    ...
    if not force_local:
        for url in WB_IEG_URLS:
            try:
                ...
                log.info(f"  [WB] ✓ {len(df):,} rows from live API")
    ...
    if os.path.exists(WB_LOCAL):
        ...
        if max_rows: df = df.head(max_rows)
        log.info(f"  [WB] ✓ {len(df):,} rows from local cache")
        return df
    ...
    for path in paths:
        if os.path.exists(path):
            try:
                ...
                loaded = True
                log.info(f"  [AID] ✓ {len(df):,} rows loaded")
                break
            except Exception as e:
...
def _load_aiddata_csv(max_rows=None):
    ...
    if os.path.exists(AIDDATA_CSV_LOCAL):
        ...
        if max_rows: df = df.head(max_rows)
        log.info(f"  [AID] ✓ {len(df):,} rows from CSV cache")
        return df
    return None
